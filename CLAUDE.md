...
- **Verbatim crown jewels.** The following blocks survived real projects and are preserved word-for-word unless I explicitly say otherwise: the DESIGN EXCERPT SCOPE rules and AC drift guard in skills/user-stories, the LOGBOOK/Changelog formats in references/org-conventions.md, the Changelog-requires-body-edit invariant and five-step flow in skills/update-design, and every STOP boundary. Rewording for "clarity" is how these die.
- **Single source of truth.** LOGBOOK, timestamp, checkbox, and Changelog conventions change in references/org-conventions.md and nowhere else. If a skill needs a format tweak, the conventions file changes and the skills keep pointing at it.

---
name: phase-close
description: Closes the open batch in PLAN.org — verification, document reconciliation, advisory pass, the gate record, and the batch retro (per-story session metrics appended to retro.org). Use when a batch or phase is done, when the user says they are ready to gate, or as a pre-milestone check before starting the next batch.
model: opus
---
...
1. Run the repo's declared `Verify command:` (CLAUDE.md); if the line is absent, default per weight class: work-grade runs `mix check`, personal-mvp runs `mix format --check-formatted` and `mix test`. All must pass; fix failures before continuing (test or format fixes only — behavior changes at this stage go through a story). If the PRD declares gate criteria (e.g. an on-device checklist), run them too — where a criterion is a manual check, present it to the human for sign-off; never fake a result. The same holds for the automated checks: a red run is never recorded as passing by reinterpreting or narrowing the check, and a failure that cannot be fixed within this step's bounds is surfaced to the human, who may rule a documented deferral — recorded in the gate record, never silently absorbed.
...
7. Retro: run `python3 ${CLAUDE_PLUGIN_ROOT}/tools/story-metrics.py` with every story file in the batch as arguments (from the repo root, so the script can match sessions to this repo and read its commits). Append a `* Batch <name> — <gate date>` heading to `retro.org` in the initiative directory, creating the file with a `#+title:` line if absent, with three subheadings. `** Metrics` holds the script's table and its footnote verbatim. `** Observations` holds short factual bullets drawn from the script's timeline, not from memory of the sessions: fix loops (a story re-verified after a NOT MET), human questions asked, deferred discoveries, a stage that ran far longer than its siblings, a story whose transcript is missing on this machine. The table's Note column records story files a session referenced, reads included — it is not evidence that a commit edited them; check git before claiming propagation. The timeline itself is never pasted. `** Retro` is left empty — it belongs to the human, and no skill writes there. If the script finds no transcripts at all, write the Metrics table as printed (every row says so) and move on; the retro records what this machine can see, and a missing row is a fact, not a failure.

8. Report a short summary to the user: what shipped, notable pivots (from the Decision Log), and the advisories withdrawn/promoted.

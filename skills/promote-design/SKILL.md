...
5. Before presenting DESIGN.org for approval, scrub its body and Decision Log entries for brainstorm-spec references: grep for "spec §", "brainstorm", and the spec filename pattern. Report every hit, then rewrite each to self-contained wording per org-conventions' Document chain & version control section (the rule lives there, not here). This is a report-then-fix step — list what was found and how each was reworded, never a silent rewrite.
